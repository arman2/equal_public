...
{
    public class PubStream : DP_Line
    {
        ...
        {
            if (endpoint is Publish)
            {
                return true;
            }
            if (endpoint is Receive)
            {
                return true;
            }
            return false;
        ...
        public PubStream(DomainProDesigner.DP_ConnectionSpec newRole1, DomainProDesigner.DP_ConnectionSpec newRole2) :
            base(newRole1, newRole2)
        {
            Diagram = new PubStreamStructure();
            Text = new DP_Text();
        ...
        {
            PubStream newType = new PubStream();
            newType.Diagram = new PubStreamStructure();
        ...
        {
            if (source is PubStream)
            {
              base.Copy(source);
              PubStream srcPubStream = source as PubStream;
            }
        }
    }
}
