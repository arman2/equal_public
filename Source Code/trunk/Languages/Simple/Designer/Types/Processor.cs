...
{
    public class Processor : DP_Shape
    {
        ...
        public Processor(Point startLocation) :
            base(startLocation)
        {
            Diagram = new ProcessorStructure();
            Text = new DP_Text();
        }
        ...
        {
            Processor newType = new Processor();
            newType.Diagram = new ProcessorStructure();
        ...
        {
            if (source is Processor)
            {
              base.Copy(source);
              Processor srcProcessor = source as Processor;
        Workers = srcProcessor.Workers;
        Queues = srcProcessor.Queues;
            }
        }
    }
}
